...
public class Application {
    ...
    public static void main(String[] args) {
        ...
        * 프로그램을 만들 때 적용하여 새로은 세계를 창조하는 방법론
        * 객체 간의 상호작용을 한다는 사실은 현실 세계와 닮아있지만
        * 현실 세계와 다른 점은 의읜화 기법이 적용되어
        * 현실 세계에서 불가능한 무생물이나 개념같은 존재들도 하나의 주체로
        * 본인의 상태를 스스로 제어하고 활동하도록 한다는 것(캡슐화)
        * 모든 객체는 책임을 가지고 자신의 역할을 수행해야 함
        * 또한 하나의 객체는 너무 많은 일을 수행하는 것이 아닌 적절한 책임을 가지고 있어야 함(단일 책임의 원칙)
        * */

        /*
        * 객체를 설계할 떄 복잡한 현실 세계를 그대로 반영하는 것은 너무 방대하고 복잡하기 때문에
        * 현실 세계를 프로그램의 목적에 맞게 단순화하는 추상화라는 기법을 적용하게 됨
        * */
        ...
        * 객체의 재사용성이 증가하면 중복 작성되는 코드를 줄일 수 있으며, 오류 발생 가능성을 감소시키고 유지보수성을 증가시킴
        * */

        /*
        * 객체와 객체의 상호작용이란?
        * 객체와 객체는 메세지를 통해 서로 상호작용을 함
        * 메세지(메소드 호출)를 보내는 쪽을 송신자라고 하고 메세지를 받는 쪽을 수신자라고 함
        * 수신자는 메세지를 전달받아 그 메세지에 해당하는 내용을 처리하는 방법을 스스로 결정하고,
        * 그 결정한 밥법대로 처리할 명령어들을 순차적으로 기술한 것을 메소드라고 함
        * 상호작용에 중요한 것은 메세지를 송수신하고 그 방법을 기술하여 메세지에 응답하는 상호작용을 설계하는 것
        * 따라서 객체를 설계할 때 속성은 부가적이며 적절한 책임을 가진 객체들 간의 주고받는 메세지를 이용해
        * 상호작용하는 것을 설계하는 것을 의미함
        * 즉, 필드보다는 메소드를 중점으로 추상화기법을 적용하여 객체를 설계하고
        * 그 객체를 구현하는 방법으로 설계한 대로 클래스를 작성하는 것을 추상화라고 부름
        * */
