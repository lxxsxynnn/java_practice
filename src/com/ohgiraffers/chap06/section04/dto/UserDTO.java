...
public class UserDTO {
    ...
    * 자바 코드를 모르는 웹 퍼블리셔들도 자바 코드를 사용할 수 있도록 태그 형식으로 지원하는 문법을 의미
    * 그 때 사용할 수 있도록 규칙을 지정해놓은 java 클래스를 자바 빈(java bean)이라고 부름
    ...
    * 2. 멤버변수의 접근 제어자는 private으로 선언해야 함
    * 3. 기본 생성자가 명시적으로 존재해야 함(매개변수 생성자는 선택사항)
    * 4. 모든 멤버변수에 접근 가능한 설정자(setter)와 접근자(getter)가 public으로 작성되어 있어야 함
    * 5. 직렬화(Serializable 구현)가 되어있어야 함(선택사항)
    * */
    ...
    /*
    * 기본 생성자 명시적으로 작성
    * 매개변수가 있는 생성자를 선택적으로 추가할 수 있도록 기본 생성자를 명시함
    * (명시하지 않고 추후 매개변수가 있는 생성자를 추가할 시 에러 발생 가능성이 있기 때문)
    * */
    public UserDTO(){}

    /*
    * 매개변수가 있는 생성자는 선택사항임
    * 필요에 따라 만들지만 일반적으로 가장 많이 사용되는 생성자는 모든 필드를 초기화하는 생성자
    * */
    ...
    * 모든 필드에 대해 각 설정자와 접근자를 작성하며 접근제한 범위는 public으로 설정
    * */
    /* 설정자(setter) */
