...
public class Application1 {
    ...
    public static void main(String[] args) {
        ...
        * 즉, 2차원 배열은 1차원 배열 여러개를 하나로 묶어서 관리하는 배열을 뜻함
        * 더 많은 차원의 배열을 사용할 수 있지만 일반적으로 2차원 배열보다 더 높은 차원의 배열은 사용 빈도가 현저히 적음(인지 범위 초과)
        * */
        ...
        * */
        ...
        /* 2. 여러 개의 1차원 배열의 주소를 관리하는 배열을 생성(heap) */
        /*
        * heap에 공간을 할당할 때는 new 연산자를 이용
        * 배열의 주소를 관리하는 배열의 길이는 앞 부분에 정수형으로 표기하며 반드시 크기를 지정해줘야 함
        * */
        ...
        /*
        * 참고 : 위에서 진행한 2, 3번을 동시에 진행할 수도 있음
        * 앞 부분 정수는 주소를 관리하는 배열의 크기, 뒷 부분 정수는 각 인덱스에 할당하는 배열의 길이임
        * 관리하는 여러 개의 배열의 길이가 동일한 경우 아래와 같이 한 번에 할당할 수도 있음
        * 하지만 여러 개의 배열의 길이가 각각 다른 경우 위에서 했던 방식처럼 인덱스별로 배열을 따로 할당해주어야 함
        * 서로 같은 길이의 여러 개 배열을 하나로 묶어 관리하는 2차원 배열을 정변 배열 이라고 하며
        * 서로 길이가 같지 않은 여러 개의 배열을 하나로 묶어 관리하는 2차원 배열을 가변 배열 이라고 함
        * */
        iarr2 = new int[3][5];

        /* 4. 각 배열의 인덱스에 차례로 접근해서 값 출력 */
        /* 할당 후에 아무 값을 대입하지 않아도 heap 영역에는 값이 없는 상태로 공간을 생성할 수 없기 때문에 기본값이 들어있음 */
        ...
        /*
        * 방금 출력한 내용에서 규칙 찾아보기
        * 배열의 처음과 끝을 차례로 접근하여 출력하는 구문 자체를 3번 반복하고 있음
        * 따라서 한 개 인덱스가 가진 배열의 값을 출력하는 반복문을 3번 반복하는 반복문을 작성해보자 */
        for(int i = 0; i < iarr1.length; i++){

            /* 각 인덱스의 배열값을 차례대로 출력하는 반복문 */
