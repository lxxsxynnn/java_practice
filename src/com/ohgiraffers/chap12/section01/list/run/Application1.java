...
public class Application1 {
    public static void main(String[] args) {
        ...
        * 하지만 Map은 구조 상의 차이로 Collection 인터페이스에서 정의하고 있지 않다.
        * */
        ...
        * 크기 변경(새로운 더 큰 배열 만들고 옮기기), 요소의 추가, 삭제, 정렬 기능들을 미리 메서드로 구현해서 제공하고 있다.
        ...
        * List 인터페이스의 하위의 다양한 구현체들로 타입 변경이 가능하기 때문에
        ...
        * */
        ...
        /*
        * toString 메서드가 overriding 되어 있다.
        * 출력해보면 저장 순서를 유지하고 있다.
        * */
        System.out.println("alist : " + alist);

        /*
        * arrayList의 크기는 size() 메서드로 확인할 수 있다.
        * 단, size() 메서드는 배열의 크기가 아닌 요소의 갯수를 반환한다.
        * 내부적으로 관리되는 배열의 사이즈는 외부에서 알 필요가 없기 때문에 기능을 제공하지 않는다.
        * */
        System.out.println("alist의 size : " + alist.size());

        /* 내부 배열에 인덱스가 지정되어 있기 때문에 for문으로 접근도 가능하다. */
        for (int i = 0; i < alist.size(); i++) {

            /* 인덱스에 해당하는 값을 가져올 때는 get() 메서드를 사용한다. */
        ...
        * */
        ...
        /* 저장된 위치의 값을 수정할 때에도 인덱스를 활용할 수 있으며 set() 메서드를 사용한다. */
        alist.set(1, Boolean.valueOf(true));
        System.out.println("alist : " + alist);

        /* 모든 컬렉션 프레임워크 클래스는 제너릭 클래스로 작성되어 있다. */
        List<String> stringList = new ArrayList<>();
        stringList.add("apple");
        /* 제너릭 타입을 지정하면 지정한 타입 외의 인스턴스는 저장하지 못한다. */
//        stringList.add(123);
        ...
        * Collection에서 사용되는 기능들을 static 메서드로 구현한 클래스이며
        * 인터페이스명 뒤에 s가 붙은 클래스들은 관례상 비슷한 방식으로 작성된 클래스를 의미하게 된다.
        * */
        Collections.sort(stringList);

        /* sort 메서드를 사용하면 list가 오름차순 정렬 처리 된 후 정렬 상태가 유지된다. */
        ...
        * 제너릭을 적용하는 것이 좋다.
        * LinkedList 타입으로 형변환 한 후 decendingIterator() 메서드를 사용하면
        * 내림차순으로 정렬된 Iterator 타입의 목록으로 반환을 해 준다.
        * */
